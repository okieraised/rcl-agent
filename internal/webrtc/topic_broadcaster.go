...
func (b *topicBroadcaster) AddViewer(ctx context.Context, node *humble.Node, topic string, out chan<- []byte) {
	b.mu.Lock()
	defer b.mu.Unlock()
...
func (b *topicBroadcaster) RemoveViewer(topic string, out chan<- []byte) {
	...
	if len(lst) == 0 {
		if cancel, ok := b.cancel[topic]; ok {
			log.Default().Info("Stopping ROS subscriber for topic " + topic)
			cancel()

...
func init() {
	humble.RegisterMessage("rcl_interfaces/Log", LogTypeSupport)
	humble.RegisterMessage("rcl_interfaces/msg/Log", LogTypeSupport)
}
const (
...
func (t *Log) CloneMsg() humble.Message {
	return t.Clone()
}
...
func (t *Log) GetTypeSupport() humble.MessageTypeSupport {
	return LogTypeSupport
}

// LogPublisher wraps humble.Publisher to provide type safe helper
// functions
...
// Log
func NewLogPublisher(node *humble.Node, topicName string, options *humble.PublisherOptions) (*LogPublisher, error) {
...
// LogSubscription wraps humble.Subscription to provide type safe helper
// functions
...
type LogSubscriptionCallback func(msg *Log, info *humble.MessageInfo, err error)

// NewLogSubscription creates and returns a new subscription for the
// Log
func NewLogSubscription(node *humble.Node, topicName string, opts *humble.SubscriptionOptions, subscriptionCallback LogSubscriptionCallback) (*LogSubscription, error) {
...
func (s *LogSubscription) TakeMessage(out *Log) (*humble.MessageInfo, error) {
...
var LogTypeSupport humble.MessageTypeSupport = _LogTypeSupport{}

type _LogTypeSupport struct{}

func (t _LogTypeSupport) New() humble.Message {
	return NewLog()
}
...
func (t _LogTypeSupport) AsCStruct(dst unsafe.Pointer, msg humble.Message) {
	m := msg.(*Log)
	...
	humble.StringAsCStruct(unsafe.Pointer(&mem.msg), m.Msg)
	humble.StringAsCStruct(unsafe.Pointer(&mem.file), m.File)
	humble.StringAsCStruct(unsafe.Pointer(&mem.function), m.Function)
	mem.line = C.uint32_t(m.Line)
}

func (t _LogTypeSupport) AsGoStruct(msg humble.Message, ros2_message_buffer unsafe.Pointer) {
	m := msg.(*Log)
	...
	humble.StringAsGoStruct(&m.Msg, unsafe.Pointer(&mem.msg))
	humble.StringAsGoStruct(&m.File, unsafe.Pointer(&mem.file))
	humble.StringAsGoStruct(&m.Function, unsafe.Pointer(&mem.function))

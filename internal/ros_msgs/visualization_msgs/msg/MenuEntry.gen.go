...
func init() {
	humble.RegisterMessage("visualization_msgs/MenuEntry", MenuEntryTypeSupport)
	humble.RegisterMessage("visualization_msgs/msg/MenuEntry", MenuEntryTypeSupport)
}
...
func (t *MenuEntry) CloneMsg() humble.Message {
	return t.Clone()
}
...
func (t *MenuEntry) GetTypeSupport() humble.MessageTypeSupport {
	return MenuEntryTypeSupport
}

// MenuEntryPublisher wraps humble.Publisher to provide type safe helper
// functions
...
// MenuEntry
func NewMenuEntryPublisher(node *humble.Node, topicName string, options *humble.PublisherOptions) (*MenuEntryPublisher, error) {
...
// MenuEntrySubscription wraps humble.Subscription to provide type safe helper
// functions
...
type MenuEntrySubscriptionCallback func(msg *MenuEntry, info *humble.MessageInfo, err error)

// NewMenuEntrySubscription creates and returns a new subscription for the
// MenuEntry
func NewMenuEntrySubscription(node *humble.Node, topicName string, opts *humble.SubscriptionOptions, subscriptionCallback MenuEntrySubscriptionCallback) (*MenuEntrySubscription, error) {
...
func (s *MenuEntrySubscription) TakeMessage(out *MenuEntry) (*humble.MessageInfo, error) {
...
var MenuEntryTypeSupport humble.MessageTypeSupport = _MenuEntryTypeSupport{}

type _MenuEntryTypeSupport struct{}

func (t _MenuEntryTypeSupport) New() humble.Message {
	return NewMenuEntry()
}
...
func (t _MenuEntryTypeSupport) AsCStruct(dst unsafe.Pointer, msg humble.Message) {
	m := msg.(*MenuEntry)
	mem := (*C.visualization_msgs__msg__MenuEntry)(dst)
	mem.id = C.uint32_t(m.Id)
	mem.parent_id = C.uint32_t(m.ParentId)
	humble.StringAsCStruct(unsafe.Pointer(&mem.title), m.Title)
	humble.StringAsCStruct(unsafe.Pointer(&mem.command), m.Command)
	mem.command_type = C.uint8_t(m.CommandType)
}

func (t _MenuEntryTypeSupport) AsGoStruct(msg humble.Message, ros2_message_buffer unsafe.Pointer) {
	m := msg.(*MenuEntry)
	mem := (*C.visualization_msgs__msg__MenuEntry)(ros2_message_buffer)
	m.Id = uint32(mem.id)
	m.ParentId = uint32(mem.parent_id)
	humble.StringAsGoStruct(&m.Title, unsafe.Pointer(&mem.title))
	humble.StringAsGoStruct(&m.Command, unsafe.Pointer(&mem.command))
